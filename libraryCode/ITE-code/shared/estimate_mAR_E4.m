function [x_innovation_hat,Fx_hat] = estimate_mAR_E4(x,L,method)
%function [x_innovation_hat,Fx_hat] = estimate_mAR_E4(x,L,method)
%mAR estimation on observation x via the 'subspace', 'subspace-LL', or 'LL' method.
%
%INPUT:
...
%initialization:
    ...
    disp(strcat(['mAR(',num2str(L),')-fit, method: ',method,'; started.']));
    ...
    switch method
        ...
        case 'subspace-LL'
            ...
            %LL:
                ...
                [theta_opt] = e4min('lfmiss',theta_temp,'',din0,x.');%'lfmiss'<->missing data
        case 'LL'
            maxiter_LL = 100;
            sete4opt('maxiter',maxiter_LL);%set the optimization parameters: the iteration number
            [theta_opt] = e4min('lfmiss',theta_temp,'',din0,x.');%'lfmiss'<->missing data
...
disp(strcat(['mAR(',num2str(L),')-fit, method: ',method,'; ready.']));
