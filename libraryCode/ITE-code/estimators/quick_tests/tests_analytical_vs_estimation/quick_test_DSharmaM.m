%function [] = quick_test_DSharmaM()
%Quick test for Sharma-Mittal divergence estimators: analytical expression vs estimated value as a function of the sample number. In the test, normal variables are considered. See also 'analytical_value_DSharmaM.m'.
...
%parameters:
    distr = 'normal'; %fixed
    alpha_D = 0.7; %parameter of Sharma-Mittal divergence; >0, \ne 1
    beta_D = 0.5; %parameter of Sharma-Mittal divergence, \ne 1
    d = 1; %dimension of the distribution
    num_of_samples_v = [1000:2000:30*1000]; %sample numbers used for estimation
    %estimator (of Sharma-Mittal divergence), base:
        cost_name = 'SharmaM_kNN_k'; %d>=1
        %cost_name = 'SharmaM_expF'; %d>=1; 0<alpha<1 <-> convexity of the natural space
        
%initialization:
    num_of_samples_max = num_of_samples_v(end);
    L = length(num_of_samples_v);
    co = D_initialization(cost_name,1,{'alpha',alpha_D,'beta',beta_D}); %{'alpha',alpha_D,'beta',beta_D}: set the 'alpha' and 'beta' fields
    D_hat_v = zeros(L,1); %vector of estimated Sharma-Mittal divergences
    ...
    switch distr
        case 'normal'
            ...
            %analytical value of Renyi divergence:
                ...
                D = analytical_value_DSharmaM(distr,distr,alpha_D,beta_D,par1,par2);
        otherwise
            error('Distribution=?');            
    end
...
%plot:
    ...
    ylabel('Sharma-Mittal divergence');    
