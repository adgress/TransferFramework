function [co] = D_initialization(cost_name,mult,post_init)
...
%Here, we make use of the naming convention 'D<name>_initialization':
   ...
   elseif nargin == 3
       eval(['co=D',cost_name,'_initialization(mult,post_init);']); %example: co = DRenyi_kNN_k_initialization(mult,post_init);
   else
       error('Wrong number of arguments.');
   end
   %attach a function handle to the cost object; it will be used in 'D_estimation.m'
       co.function_handle = eval(strcat('@D',co.name,'_estimation')); %example: co.function_handle = @DRenyi_kNN_k_estimation
