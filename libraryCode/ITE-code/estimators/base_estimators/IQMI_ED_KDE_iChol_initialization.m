function [co] = IQMI_ED_KDE_iChol_initialization(mult,post_init)
%function [co] = IQMI_ED_KDE_iChol_initialization(mult)
%function [co] = IQMI_ED_KDE_iChol_initialization(mult,post_init)
%Initialization of the Euclidean distance based quadratic mutual information (QMI-ED) approximate estimator, applying Gaussian KDE (kernel density estimation) and incomplete Cholesky decomposition.
%
%Note:
...
%mandatory fields (following the template structure of the estimators to make uniform usage of the estimators possible):
    co.name = 'QMI_ED_KDE_iChol';
    co.mult = mult;
    
%other fields: 
    co.sigma = 0.5; %std in the RBF (Gaussian) kernel
