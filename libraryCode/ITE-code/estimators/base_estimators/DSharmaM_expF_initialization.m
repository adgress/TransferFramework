function [co] = DSharmaM_expF_initialization(mult,post_init)
%function [co] = DSharmaM_expF_initialization(mult)
%function [co] = DSharmaM_expF_initialization(mult,post_init)
%Initialization of the exponential family based Sharma-Mittal divergence estimator (maximum likelihood + analytical formula associated to the chosen exponential family).
%
%Note:
...
%other fields:
    ...
    co.alpha = 0.8; %0<alpha<1 <-> convexity of the natural space
    co.beta = 0.6; %beta: not 1.
    %The Sharma-Mittal divergence (D_{SM,alpha,beta}) equals to the
    %  1)Tsallis divergence (D_{T,alpha}): D_{SM,alpha,beta} = D_{T,alpha}, if alpha = beta.
    %  2)Kullback-Leibler divergence (D): D_{SM,alpha,beta} -> D, as (alpha,beta) -> (1,1).
