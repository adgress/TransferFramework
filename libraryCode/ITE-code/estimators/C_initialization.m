function [co] = C_initialization(cost_name,mult,post_init)
...
%Here, we make use of the naming convention 'C<name>_initialization':
   ...
   elseif nargin == 3
   	   eval(['co=C',cost_name,'_initialization(mult,post_init);']); %example: co = CCE_kNN_k_initialization(mult,post_init);
   else
       error('Wrong number of arguments.');
   end
   %attach a function handle to the cost object; it will be used in 'C_estimation.m'
       co.function_handle = eval(strcat('@C',co.name,'_estimation')); %example: co.function_handle = @CCE_kNN_k_estimation
