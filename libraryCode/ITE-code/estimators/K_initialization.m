function [co] = K_initialization(cost_name,mult,post_init)
...
%Here, we make use of the naming convention 'K<name>_initialization':
   ...
   elseif nargin == 3
   	   eval(['co=K',cost_name,'_initialization(mult,post_init);']); %example: co = Kexpected_initialization(mult,post_init);
   else
       error('Wrong number of arguments.');
   end
   %attach a function handle to the cost object; it will be used in 'K_estimation.m'
       co.function_handle = eval(strcat('@K',co.name,'_estimation')); %example: co.function_handle = @Kexpected_estimation
