function [co] = H_initialization(cost_name,mult,post_init)
...
%Here, we make use of the naming convention 'H<name>_initialization':
   ...
   elseif nargin == 3
   	   eval(['co=H',cost_name,'_initialization(mult,post_init);']); %example: co = HRenyi_kNN_k_initialization(mult,post_init);
   else
       error('Wrong number of arguments.');
   end
   %attach a function handle to the cost object; it will be used in 'H_estimation.m'
       co.function_handle = eval(strcat('@H',co.name,'_estimation')); %example: co.function_handle = @HRenyi_kNN_k_estimation
