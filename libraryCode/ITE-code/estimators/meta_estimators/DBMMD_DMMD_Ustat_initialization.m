function [co] = DBMMD_DMMD_Ustat_initialization(mult,post_init)
%function [co] = DBMMD_DMMD_Ustat_initialization(mult)
%function [co] = DBMMD_DMMD_Ustat_initialization(mult,post_init)
%Initialization of the MMD (maximum mean discrepancy) divergence estimator, applying average of U-statistics, i.e., the samples are partitioned into blocks ('B').
%
%Note:
%   1)The estimator is treated as a cost object (co).
%   2)We use the naming convention 'D<name>_initialization' to ease embedding new divergence estimation methods.
%   3)This is a meta method: the U-statistics based MMD estimator can arbitrary.
%
%INPUT:
...
%mandatory fields (following the template structure of the estimators to make uniform usage of the estimators possible):
    co.name = 'BMMD_DMMD_Ustat';
    co.mult = mult;
    
%other fields:
    co.member_name = 'MMD_Ustat'; %you can change it to any U-statistics based MMD estimator
