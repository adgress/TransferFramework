function [co] = A_initialization(cost_name,mult,post_init)
...
%Here, we make use of the naming convention: 'A<name>_initialization':
   ...
   elseif nargin == 3
   	   eval(['co=A',cost_name,'_initialization(mult,post_init);']); %example: co = ASpearman1_initialization(mult,post_init);
   else
       error('Wrong number of arguments.');
   end
   %attach a function handle to the cost object; it will be used in 'A_estimation.m'
       co.function_handle = eval(strcat('@A',co.name,'_estimation')); %example: co.function_handle = @ASpearman1_estimation
