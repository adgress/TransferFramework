function [perm_ICA] = clustering_UD0_CE_general(s_ICA,ds,cost_type,co)
%function [perm_ICA] = clustering_UD0_CE_general(s_ICA,ds,cost_type,co)
%Clusters the ICA (s_ICA) elements to subspaces of given dimensions (ds)
%using (i) the 'CE' method and (ii) cost given in the cost object co of
%type cost_type. 
%
%Note: in case of a cost type being additive with respect to the coordinates (cost_type = 'Ipairwise1d'), 
%more efficient optimization is also available (see 'clustering_UD0_CE_pairadditive_wrt_coordinates.m').
%
%INPUT:
...
while (max(max(abs(P-P_old))) > tolerance_treshold) && (~const_gammas) %while P changed 'much' and the last gamma values were different in the last tolerance_last iterations
    %generate num_of_CE_samples samples and compute their costs:
        for k = 1 : num_of_CE_samples
            ...
            tours(:,k) = tour;
            costs(k) = cost_general(s_ICA(tour,:),ds,cost_type,co);
        end
